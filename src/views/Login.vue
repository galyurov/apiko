<template>
  <div class="container">
    <form class="form card" @submit.prevent="submitHandler">
      <div class="form__content">
        <div v-if="error" class="form__title error">{{error.message}}</div>
        <div v-else class="form__title">
          Login
        </div>
        <div class="form__field">
          <input v-model.trim="email" @focus="clearError"  :class="{invalid: ($v.email.$dirty && !$v.email.required) ||  ($v.email.$dirty && !$v.email.email)}" type="text" id="email" class="form__input validate">
          <label class="form__label" for="email">Email</label>
        </div>
        <div class="form__field">
          <input v-model.trim="password" @focus="clearError"  :class="{invalid: ($v.password.$dirty && !$v.password.required) ||  ($v.password.$dirty && !$v.password.minLength)}" ref="password" type="password" id="password" class="form__input validate mb0">
          <div @click="showPassword('password')" class="form__icon">
            <svg width="24" height="16" viewBox="0 0 24 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M12 0.949005C16.607 0.949005 20.852 3.26501 23.352 7.14401C23.688 7.66401 23.688 8.33601 23.352 8.85701C20.852 12.735 16.607 15.051 12 15.051C7.39298 15.051 3.14898 12.735 0.647979 8.85701C0.312979 8.33601 0.312979 7.66501 0.647979 7.14401C3.14898 3.26501 7.39298 0.949005 12 0.949005ZM12 13.851C16.198 13.851 20.065 11.741 22.344 8.20601C22.423 8.08301 22.423 7.91701 22.344 7.79401C20.064 4.25901 16.198 2.14901 12 2.14901C7.80198 2.14901 3.93598 4.25901 1.65698 7.79401C1.57698 7.91701 1.57698 8.08301 1.65698 8.20601C3.93598 11.741 7.80198 13.851 12 13.851ZM12 4.40001C10.015 4.40001 8.39998 6.01501 8.39998 8C8.39998 9.98501 10.015 11.6 12 11.6C13.985 11.6 15.6 9.98501 15.6 8C15.6 6.01501 13.985 4.40001 12 4.40001ZM9.59998 8.00001C9.59998 9.32301 10.677 10.4 12 10.4C13.323 10.4 14.4 9.32301 14.4 8.00001C14.4 6.67701 13.323 5.60001 12 5.60001C10.677 5.60001 9.59998 6.67701 9.59998 8.00001Z"
                    fill="black"/>
            </svg>
          </div>
          <label class="form__label" for="password">Password</label>
          <div class="form__help">
            Don't remember password?
          </div>
        </div>
        <div class="form__action">
          <button type="submit" class="form__btn form__submit">Continue</button>
        </div>
      </div>

    </form>
    <div class="card card__bottom">
      <span class="card__text">I have no account, </span>
      <span @click="$router.push('/register')" class="card__link">register now</span>
    </div>
  </div>
</template>

<script>
import {email,required,minLength} from 'vuelidate/lib/validators'
export default {
  name: "Login",
  data() {
    return {
      email: null,
      password: null
    }
  },
  validations: {
    email:{
      email,required
    },
    password:{
      required,
      minLength:minLength(5)
    }
  },
  methods: {
    clearError(){
      this.$store.commit('clearError')
    },
    showPassword(input) {
      if (this.$refs[input].type === 'password') {
        this.$refs[input].type = 'text'
      } else {
        this.$refs[input].type = 'password'
      }
    },
    async submitHandler() {
      if(this.$v.$invalid){
        this.$v.$touch()
        return
      }
      const formData = {
        email: this.email,
        password: this.password
      }
      try {
        await this.$store.dispatch('login',formData)
         this.$router.push('/')
        // eslint-disable-next-line no-empty
      } catch (e) {}

    }
  },
  computed:{
    error(){
      return this.$store.getters.error
    },
  }

}
</script>

<style lang="scss" scoped>
.container {
  font-family: Helvetica;
  box-sizing: border-box;
}

.form {
  margin-top: 135px;
}

.card {
  width: 425px;
  display: block;
  background: #FFFFFF;
  box-shadow: 0px 2px 42px rgb(0 0 0 / 11%);
  border-radius: 7px;
}

.container {
  display: flex;
  flex-direction: column;
  align-items: center;
}

.form__content {
  padding: 25px 24px 21px;
}

.form__title {
  margin-bottom: 50px;
  font-size: 22px;
  line-height: 25px;
  text-align: center;
  color: #282828;
}

.form__field {
  position: relative;
  display: flex;
  flex-direction: column;
}

.form__input {
  background: #F9FAFB;
  border: 1px solid #DEDEE0;
  box-sizing: border-box;
  border-radius: 5px;
  padding: 19px 41px 21px 13px;
  font-size: 16px;
  line-height: 18px;
  letter-spacing: 0.4px;
  margin-bottom: 33px;
  &::placeholder{
    color: rgba(102, 102, 102, 0.466455);
  }
}

.form__label {
  position: absolute;
  top: -15px;
  font-size: 12px;
  line-height: 14px;
  letter-spacing: 0.3px;
  text-transform: uppercase;
  color: #303030;
}

.form__help {
  text-align: end;
  font-size: 14px;
  line-height: 16px;
  letter-spacing: 0.4px;
  margin-top: 5px;
  color: #8C8C8C;
}

.form__btn {
  width: 100%;
  background: #349A89;
  border-radius: 5px;
  border: none;
  font-size: 16px;
  line-height: 18px;
  text-align: center;
  letter-spacing: 0.4px;
  padding: 19px 29px 21px;
  margin-top: 16px;
  color: #FFFFFF;
}

.card__bottom {
  margin-top: 21px;
  text-align: center;
  padding: 30px 0;
}

.card__link {
  text-transform: uppercase;
  color: #349A89;
  cursor: pointer;
}

.form__icon {
  cursor: pointer;
  position: absolute;
  top: 24px;
  right: 10px;

  &:hover svg path {
    background: none;
    fill: #349A89
  }
}

.mb0 {
  margin-bottom: 0;
}
.error{
  color: #f79483;
}
.invalid{
  border-color: #f79483;
}
@media screen and (max-width: 700px){
  .form{
    margin-top: 20px;
  }
}
</style>